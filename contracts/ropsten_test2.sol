...
contract TestSortedArray {
    ...
    function someFunction(uint[] memory sortedArray) public returns (bool valid, uint retVal) {
        ...
        for (uint i = 0; i < proofs.testcase.length; i++){
            bool comparison = (sortedArray[proofs.testcase[i]] > sortedArray[proofs.testcase[i]+1]);

            // Hash address with parameter and calculation result
            bytes32 hash = keccak256(abi.encodePacked(
                proofs.address_sender[i], proofs.testcase[i],
                sortedArray[proofs.testcase[i]], comparison));
        ...
        if (counterexample_finder != address(0)) {
            if (sortedArray[counterexample_testcase] > sortedArray[counterexample_testcase+1]) {
                // Send incentive:
    ...
    /******************************************************************************************/
    /* Debug */
    function debug_repayFund () public {
        owner.transfer(address(this).balance);
    }
    
    function debug_getProofs () public view returns (Proof memory retProofs) {
        retProofs = proofs;
    }
    
    function debug_setRunningStatus (bool status) public {
        running = status;
    }
}
